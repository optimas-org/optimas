...
def test_ax_model_manager():
    ...
    path = os.path.join(exploration_dir_path, 
    ...
    assert np.allclose(mean_axcl, mean_json, rtol=1e-2) \
         and np.allclose(mean_axcl, mean_diag, rtol=1e-2)
    ...
    # plot model for `f`
    ax1 = mm_axcl.plot_model(mname='f', 
    ...
    # plot model for `f2`
    ax2 = mm_axcl.plot_model(mname='f2', 
    ...
    parx0 = mm_axcl.ax_client.experiment.parameters['x0']
    parx1 = mm_axcl.ax_client.experiment.parameters['x1']
    x1_mid = 0.5 * (parx1.lower + parx1.upper)
    x0 = np.linspace(parx0.lower, parx0.upper, 100)
    metric_names = mm_axcl.ax_client.objective_names
    fig, axs = plt.subplots(len(metric_names), 1,
                            sharex=True)
    for i, (ax, metric_name) in enumerate(zip(axs, metric_names)):
        mean, sed = mm_axcl.evaluate_model(sample={'x0': x0},
                                        p0={'x1': x1_mid},
                                        metric_name=metric_name)
        ax.plot(x0, mean, color=f'C{i}', label=f'x1 = {x1_mid}')
        ax.fill_between(x0, mean - sed, mean + sed, color='lightgray', alpha=0.5)
