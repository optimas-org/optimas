...
def test_line_sampling_errors():
    ...
    with pytest.raises(
            AssertionError,
            match='Parameter x0 does not have a default value.'):
        gen = LineSamplingGenerator(
    ...
    # Create varying parameters.
    var1 = VaryingParameter('x0', -3, 1, default_value=0.)
    var2 = VaryingParameter('x0', -3, 1, default_value=0.)
    ...
    with pytest.raises(
            AssertionError,
            match=re.escape('Length of `n_steps` (1) and'
