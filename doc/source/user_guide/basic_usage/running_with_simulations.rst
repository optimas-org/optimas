...
- A template of the simulation script that indicates where the values
  of the variables should be placed.
...
values of the variables are introduced using
`Jinja <https://jinja.palletsprojects.com>`_ syntax, that is, using the
double-bracket notation ``{{var_name}}``, where ``var_name`` is the name of
the variable as defined in the :class:`~gest_api.vocs.VOCS` object.
...
.. code-block:: python
   ...
   def analyze_simulation(simulation_directory, output_params):
       ...
       if specified. The value of these parameters has to be given to the
