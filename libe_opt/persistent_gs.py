...
def persistent_regular_grid_search_single(H, persis_info, gen_specs, libE_info):
    ...
    lb = user_specs['lb']
    ub = user_specs['ub']
    nominal_value = user_specs['nominal_value']
    n_points = user_specs['n_steps']
    n_vars = len(lb)
    n_configs = np.sum(n_points)

    all_configs = np.ones((n_configs, n_vars)) * nominal_value
    for i in range(n_vars):
        var_vals = np.linspace(lb[i], ub[i], n_points[i])
        all_configs[np.sum(n_points[:i]):np.sum(n_points[:i])+n_points[i], i] = var_vals
    ...
    while tag not in [STOP_TAG, PERSIS_STOP]:
        ...
        if i_first > n_configs:
            ...
            H_o = np.zeros(i_last-i_first, dtype=gen_specs['out'])
