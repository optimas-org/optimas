...
def LUXlaser(energy_measured_joule, FWHM_x_um, FWHM_t_fs, lambda_laser=0.8,
             T_beamline=1, focus_factor=1, temporal_factor=1):
    energy_gauss_joule = energy_measured_joule * \
        T_beamline * focus_factor * temporal_factor
    FWHMtoSigma = 2 * np.sqrt(2 * np.log(2))
    I_0 = energy_gauss_joule / ((np.sqrt(2*np.pi))**3 *
...
if __name__ == '__main__':
    ...
    sim = Simulation(Nz, zmax, Nr, rmax, Nm, dt, zmin=zmin,
                     ...
                     initialize_ions=False, n_order=n_order, use_cuda=use_cuda,
    ...
    atoms_N = sim.add_new_species(
        q=5*e, m=14.*m_p, n=1, dens_func=dens_func_N, p_nz=p_nz_N, p_nr=p_nr_N,
        p_nt=p_nt_N, p_zmin=p_zmin, p_rmax=p_rmax)
    atoms_H = sim.add_new_species(
        q=e, m=1*m_p, n=1, dens_func=dens_func_H, p_nz=p_nz, p_nr=p_nr,
        p_nt=p_nt, p_zmin=p_zmin, p_rmax=p_rmax)
    elec = sim.add_new_species(
        q=-e, m=m_e, n=1, dens_func=dens_func_e, p_nz=p_nz, p_nr=p_nr,
        p_nt=p_nt, p_zmin=p_zmin, p_rmax=p_rmax)
    ...
    sim.diags = [
        BoostedParticleDiagnostic(
            zmin, zmax, c, dt_snapshot_lab, Ntot_snapshot_lab, gamma_boost,
            diag_period, sim.fld, species={"electrons from N": elec_from_N},
